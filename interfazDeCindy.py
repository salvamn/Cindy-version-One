...
def worker_reconocimiento():
    p = threading.Thread(target=reconocimiento_de_voz)
    p.start()
    p.is_alive()
    print(p)
...
Button(ventana, text="Hablar", image=img, command=seleccion_por_voz).place(x=300, y=150)
